package com.example.gd12_qr_b_10712
...
import com.example.gd12_qr_b_10712.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun requesStoragePermission() {
        ...
        when(requestCode){
            CAMERA_REQUEST_CODE->{
                if (grantResults.isNotEmpty()){
                    ...
                    val strogeAccepted = grantResults[1] == PackageManager.PERMISSION_GRANTED
                    if(cameraAccepted && strogeAccepted){
                        pickImageCamera()
                    }else{
            ...
            STORAGE_REQUEST_CODE->{
                if (grantResults.isNotEmpty()){
                    val strogeAccepted = grantResults[0] == PackageManager.PERMISSION_GRANTED
                    if(strogeAccepted){
                        pickImageGallery()
                    }else{
            ...
            for (barcode in barcodes){
                ...
                when (valueType){
                    ...
                    Barcode.TYPE_CONTACT_INFO->{
                        ...
                        var name = "${typeContact?.name}"
                        var phone = "${typeContact?.phones}"
